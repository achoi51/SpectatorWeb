...
function Article(props: Props) {
	...
	const handleImages = () => {
		...
		while ((match = regex.exec(text)) !== null) {
			...
			const contributor = props.article.cover_image_contributor.find( (c) => String(c._id) === contributorId );
			const contributorName = contributor ? contributor.name : "Unknown Contributor";

			const contributorLink = `\n<div id="${styles.cover_image_div}">By&nbsp;<span><span><a href="/staff/${contributor?.slug}"><span class="discrete-link">${contributorName}</span></a></span></span></div>`;

			modifiedText = modifiedText.replace(imgtag, `${imgtag} ${contributorLink}`);
		}
		...
		<div>
			...
			<main id={styles.main}>
				<article id={styles.article}>
					...
					<div
						id={styles.content}
						dangerouslySetInnerHTML={{ __html: handleImages() }}
					></div>
